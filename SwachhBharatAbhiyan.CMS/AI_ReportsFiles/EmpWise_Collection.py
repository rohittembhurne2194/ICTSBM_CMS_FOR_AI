...
df = pd.read_sql_query('select (select userName from UserMaster um where um.userId=gc.userId) EmployeeName, count(gc.houseId) as house_count from GarbageCollectionDetails gc group by gc.userid;', conn)
